...
export const FORMATION = {
  ...
  lieu: "Université Privée Baba Coulibaly, Bobo-Dioulasso",
...
// Construit un lien wa.me vers un numéro précis, avec un message pré-rempli.
// Le destinataire doit encore cliquer "Envoyer" dans WhatsApp — il n'existe
// pas de moyen gratuit d'envoyer un message WhatsApp 100% automatiquement
// sans passer par l'API Business de Meta (vérification d'entreprise requise).
export function buildWhatsAppLink(telephone, message) {
  const digits = String(telephone || "").replace(/\D/g, "");
  // Ajoute l'indicatif du Burkina Faso (226) si le numéro ne l'a pas déjà
  // (numéros locaux à 8 chiffres, ex: 70710089).
  const withCountryCode = digits.length === 8 ? `226${digits}` : digits;

...
export async function generateReceiptPdfBase64(record) {
  ...
  const navy = rgb(0.05, 0.23, 0.55);
  const orange = rgb(1, 0.48, 0.1);
  const gray = rgb(0.36, 0.38, 0.45);
  const lightLine = rgb(0.95, 0.95, 0.96);

  let y = 710;

  page.drawRectangle({ x: 40, y: y - 10, width: 60, height: 60, color: navy, borderRadius: 12 });
  page.drawText("SAHY", { x: 48, y: y + 14, size: 13, font: fontBold, color: rgb(1, 1, 1) });

  page.drawText("Reçu d'inscription", { x: 115, y: y + 18, size: 16, font: fontBold, color: navy });
  page.drawText(`${FORMATION.subtitle} — ${FORMATION.title}`, { x: 115, y: y + 2, size: 10, font, color: gray });

  y -= 90;
  page.drawRectangle({ x: 40, y, width: 515, height: 32, color: rgb(1, 0.93, 0.85), borderRadius: 8 });
  page.drawText(`N° de reçu : ${record.id}`, { x: 52, y: y + 11, size: 12, font: fontBold, color: orange });
  ...
  for (const [label, value] of lines) {
    ...
    page.drawLine({ start: { x: 40, y: y - 8 }, end: { x: 555, y: y - 8 }, thickness: 1, color: lightLine });
    y -= 26;
  }
  ...
  y -= 60;
  const footnote = `Ce reçu confirme l'inscription validée à la formation ${FORMATION.title} (du ${FORMATION.debut} au ${FORMATION.fin}, ${FORMATION.lieu}). Émis par ${FORMATION.organisateur}.`;
